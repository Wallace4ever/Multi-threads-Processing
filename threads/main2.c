...
#define top (0)
long long int sum =0,sumread1=0,sumread2=0,sumread3=0,sumread4=0,sumread5=0;
int number01=-1,number02=-1,number03=-1,number04=-1,number05=-1;
//int flag=0;  //原计划作为数组置底的标记  暂时没有实现

int buffer[NUM+5];  // 创建共享缓冲区 数组 在最后设置两个位置是留给标记的
  ...
  while (1)
{
  ...
  sem_wait(&sem_read);
  pthread_mutex_lock(&mutex);
  number04 = buffer[read_i];
  read_i++;
  pthread_mutex_unlock(&mutex);
  sem_post(&sem_write);
  sumread4 += 1;
  if (number04 == 0) break;    //判断数组是否已置底
  ...
  while (1)
{
  ...
  sem_wait(&sem_read);
  pthread_mutex_lock(&mutex);
  number05 = buffer[read_i];
  read_i++;
  pthread_mutex_unlock(&mutex);
  sem_post(&sem_write);
  sumread5 += 1;
  if (number05 == 0) break;    //判断数组是否已置底
...
int main(void)
{
  ...
  pthread_t th_write, th_reader1,th_reader2,th_reader3,th_reader4,th_reader5;
  void * retval;
  int i,readers;
...
//创建1个生产者和2个消费者线程
  ...
  pthread_create(&th_reader3, NULL, reader3, NULL);
  pthread_create(&th_reader4, NULL, reader4, NULL);
  pthread_create(&th_reader5, NULL, reader5, NULL);

//销毁所有线程
  (void)pthread_join(th_write, &retval);
  (void)pthread_join(th_reader1, &retval);
  (void)pthread_join(th_reader2, &retval);
  (void)pthread_join(th_reader3, &retval);
  (void)pthread_join(th_reader4, &retval);
  (void)pthread_join(th_reader5, &retval);

//输出写入总量 与 取出总量 （因为设为了全局变量 不受线程销毁的影响）
  printf(" writer sum is %lld \n", sum);
  printf("reader1 sum is %lld \n", sumread1);
  printf("reader2 sum is %lld \n", sumread2);
  printf("reader3 sum is %lld \n", sumread3);
  printf("reader4 sum is %lld \n", sumread4);
  printf("reader5 sum is %lld \n", sumread5);
