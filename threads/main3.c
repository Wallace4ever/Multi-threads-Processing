...
int read_i1,read_i2,write_i; // 数组中标记读写的位置
  ...
  while (read_i1<=50000000)
{
  ...
  sem_post(&sem_write);
  sumread1 += 1;
  if (read_i1==50000000) break;    //判断数组是否已置底
  //printf("reader1 %d\n", number01);
  ...
  while (read_i2<=100000000)
{
  ...
  sem_post(&sem_write);
  sumread2 += 1;
  if (read_i2 == 100000000) break;    //判断数组是否已置底
...
int main(void)
{
  ...
  read_i1 = 0;
  read_i2 = 50000001;                         //初始化消费者数组指针（其实就是数组中的标记）
